...
int main(int argc, char** argv) {
    if (argc != 2) {
        std::cout << "Usage: ./cfsd [config file path]" << std::endl;
        return 1;
    }
    ...
    int samplingRate = cfsd::Config::get<int>("samplingRate");
    int cameraFrequency = cfsd::Config::get<int>("cameraFrequency");
    int rate =  (samplingRate % cameraFrequency) ? (samplingRate / cameraFrequency + 1) : (samplingRate / cameraFrequency);
