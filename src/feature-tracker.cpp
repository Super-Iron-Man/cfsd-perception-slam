...
, _verbose(verbose) {
    ...
    _useCircularMatch = Config::get<bool>("useCircularMatch");
...
void FeatureTracker::internalMatch(const cv::Mat& imgLeft, const cv::Mat& imgRight, cv::Mat& descriptorsMat) {
	...
	if (_verbose) std::cout << "(internal) GMS matching inliers: " << num_inliers << ", all matches: " << matches.size() << std::endl;
...
void FeatureTracker::externalTrack() {    
    ...
    // The first frame will be set as keyframe.
    if (_pFeatures.empty()) {
        // _curFeatureMask all true.
        return;
    }
	...
	if (_verbose) std::cout << "(external left) GMS matching inliers: " << num_inliers << ", all matches: " << matchesL.size() << std::endl;
    ...
    // Perform circular matching: curLeft <=> histLeft <=> histRight <=> curRight <=> curLeft.
    if (_useCircularMatch) {
        // Store the correspondence map <queryIdx, trainIdx> of 'left' matching.
        std::map<int,int> mapCurHist;

        // Collect left cur-hist inlier matches.
        ...
        if (_verbose) std::cout << "(external right) GMS matching inliers: " << num_inliers << ", all matches: " << matchesR.size() << std::endl;

        // Collect right cur-left inlier matches and then circular matches.
        for (unsigned i = 0; i < vbInliersR.size(); ++i) {
            if (vbInliersR[i] == true) {
                ...
                if (search != mapCurHist.end() && search->second == m.trainIdx) {
                    ...
                    processMatching(m, uniqueFeature, mapPointIDs, descriptors);
                }
            }
        }
        std::cout << "# circular matches: " << _matchedFeatureIDs.size() << std::endl; 
    }
    // The possible problem of circular matching is that the valid matches would be too few to do BA.
    else {
        for (unsigned i = 0; i < vbInliersL.size(); ++i) {
            // Collect left cur-hist inlier matches.
            if (vbInliersL[i] == true) {
...
void FeatureTracker::processMatching(const cv::DMatch& m, std::unordered_map<size_t, bool>& uniqueFeature, std::vector<size_t>& mapPointIDs, cv::Mat& descriptors) {
